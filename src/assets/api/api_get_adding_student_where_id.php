<?php
  ...
  $auth = $headers['Authorization'];
    ...
    $sql = "SELECT * FROM (
        SELECT adding_withdrawing.Add_With_ID, adding_withdrawing.Student_ID, student.Student_Name, adding_withdrawing.Teacher_ID, teacher.Teacher_Name, adding_withdrawing.Add_Withdraw, adding_withdrawing.Subject_ID, subject.Subject_Name, adding_withdrawing.Instructor, adding_withdrawing.Date, adding_withdrawing.Year_ID, year.Year_Name, adding_withdrawing.Term, adding_withdrawing.Status 
    ...
    ) AS adding_withdrawing WHERE Student_ID = '$auth' AND Status='อนุมัติ' ";
  ...
  if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(strlen($txtSearch)>0){
      $sql .= " AND Add_With_ID = '$txtSearch' || 
        Student_ID like '%$txtSearch%' || 
        Year_ID like '%$txtSearch%'
      ";
    }
  }

	$sql .= " ORDER BY Add_With_ID";
