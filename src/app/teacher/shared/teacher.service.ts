...
export class TeacherService {
  ...
  getTeachers(): Observable<any[]>{
    return this.http.get<any>(environment.apiUrl + '/api_get_teacher.php');
  }

  //สร้าง function สำหรับเรียกดูข้อมูลอาจารย์ทั้งหมด
  getTeacher(formValue: any): Observable<any[]>{
    const apiHeader = { 'Content-Type': 'application/json' };
    return this.http.post<any>(environment.apiUrl + '/api_get_teacher.php', formValue, { headers: apiHeader });
  }

  //สร้าง function สำหรับเพิ่มข้อมูลอาจารย์
  insertTeacher(formValue: any): Observable<any>{
    const apiHeader = { 'Content-Type': 'application/json' };
    return this.http.post<any>(environment.apiUrl + '/api_insert_teacher.php', formValue, { headers: apiHeader });
  }
